...
async def create_server_task(interaction, ram, cpu, os, password):
    ...
    # Validate OS
    os_image = {"ubuntu-22.04": "ubuntu:22.04", "ubuntu-20.04": "ubuntu:20.04", "debian-12": "debian:12"}
    image = os_image.get(os, "ubuntu:22.04")
    ...
    if ssh_session_line:
        ...
        await interaction.followup.send(embed=discord.Embed(
            description="❌ SSH session failed. Contact support.",
            color=0xff0000
        ))
...
@bot.tree.command(name="deploy", description="Create a VPS with custom RAM, CPU, and OS")
@app_commands.describe(
    ram="Amount of RAM (e.g., 8GB, 16GB)",
    cpu="Number of CPU cores (e.g., 2, 4, 8)",
    os="Choose OS: ubuntu-22.04, ubuntu-20.04, debian-12",
    password="Set a root password"
)
async def deploy(interaction: discord.Interaction, ram: str, cpu: str, os: str, password: str):
    await create_server_task(interaction, ram, cpu, os, password)
